// var socket = io("http://localhost:3040");
const socket = io("http://localhost:3040", { transports: ["websocket"] });

  // Listen for the "updateStudents" event from the server
  socket.on('updateStudents', (data) => {
	// Alert all clients when a new student is added
	alert(data.message);
	
	// Optionally, you can also update the UI with the new student data
	console.log("New student added:", data.student);
	loadStudentData();  // Reload the student table to reflect the new student
  });
